...
class ExpiringReportController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null)
    {
        $this->title = 'Expiring Contracts Report';
        $this->print = 'catalyst_report_expiring_print';
        $this->csv = 'catalyst_report_expiring_csv';
        $this->filter = 'catalyst_report_expiring_summary_filter';

        // get params
        $params = $this->getViewParams('', 'catalyst_report_expiring_summary');
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = 'select s from CatalystServiceBundle:ServiceOrder s where s.date_expire >= :date_from and s.date_expire <= :date_to';
