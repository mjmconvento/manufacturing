...
class MandatoryReportController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null, $visit = null)
    {
        ...
        $this->print = 'catalyst_report_mandatory_print';
        $this->csv = 'catalyst_report_mandatory_csv';
        $this->filter = 'catalyst_report_mandatory_summary_filter';

        // get params
        $params = $this->getViewParams('', 'catalyst_report_mandatory_summary');
    ...
    protected function fetchData($date_from, $date_to, $visit = null)
    {
        ...
        if ($visit == 1)
        { 
            $query = 'select sv from CatalystServiceBundle:SVEntry sv join sv.service_order so where so.date_issue >= :date_from and so.date_issue <= :date_to and sv.default_visit > sv.number_of_visits';
        }
        ...
        {
            $query = 'select sv from CatalystServiceBundle:SVEntry sv join sv.service_order so where so.date_issue >= :date_from and so.date_issue <= :date_to';            
        }
