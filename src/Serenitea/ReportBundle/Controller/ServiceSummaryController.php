...
class ServiceSummaryController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null)
    {
        $this->title = 'Service Summary Report';
        $this->print = 'catalyst_report_service_print';
        $this->csv = 'catalyst_report_service_csv';
        $this->filter = 'catalyst_report_service_summary_filter';
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = 'select s from CatalystServiceBundle:ServiceOrder s where s.date_issue >= :date_from and s.date_issue <= :date_to';
