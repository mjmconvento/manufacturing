...
class PendingSchedulesController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null)
    {
        $this->title = 'Pending Schedules Report';
        $this->print = 'catalyst_report_pending_print';
        $this->csv = 'catalyst_report_pending_csv';
        $this->filter = 'catalyst_report_pending_summary_filter';

        // get params
        $params = $this->getViewParams('', 'catalyst_report_pending_summary');
    ...
    {       
        ...
        $this->title = 'Pending Schedules Report';
        $params = $this->getViewParams('', 'catalyst_report_service_summary');
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = 'select s from CatalystServiceBundle:ServiceOrder s where s.date_issue >= :date_from and s.date_issue <= :date_to and s.status_id = :issued';
